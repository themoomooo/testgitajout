...
class Tail {
    ...
    move(x, y,snakePartNumber=0) {
        
        if (this.snakeParts.length > 0 && snakePartNumber in this.snakeParts) {
            let oldX = this.snakeParts[snakePartNumber].x;
            let oldY = this.snakeParts[snakePartNumber].y;
            this.snakeParts[snakePartNumber].move(x,y); 
            return this.move(oldX, oldY, tailPartNumber+1)
        }
    }
}
...
class Snake {
    ...
    move(key) {
        ...
        if (this.x==this.food.getfood().x.animVal.value && this.y==this.food.getfood().y.animVal.value) {
            let snakePart = new SnakePart(this.food.x, this.food.y,'black')
            this.snake.tail.add(snakePart);
